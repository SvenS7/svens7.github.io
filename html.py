    ...
    for root, dirs, _ in os.walk(OUTPUT_DIR):
        ...
        for base_name, resolution_files in images.items():
            ...
            for res_name, file_path in sorted_resolutions:
                html_lines.append(
                    f"        <source srcset='{OUTPUT_DIR}{file_path}' media='(max-width: {res_name[:-1]}px)'>"
                )

            # Add fallback <img> tag
            fallback_image = sorted_resolutions[-1][
                1
            ]  # Use the largest resolution as fallback
            html_lines.append(
                f"        <img src='{OUTPUT_DIR}{fallback_image}' alt='{base_name}' loading='lazy'>"
            )
            html_lines.append(f"    </picture>")
